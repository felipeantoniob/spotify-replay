...
const spotifyMiddleware = t.middleware(async ({ ctx, next }) => {
  ...
  const spotifyApi = initializeSpotifyApi(ctx.session.accessToken, ctx.session.refreshToken)
...
export const spotifyProcedure = t.procedure.use(enforceUserIsAuthed).use(spotifyMiddleware)
